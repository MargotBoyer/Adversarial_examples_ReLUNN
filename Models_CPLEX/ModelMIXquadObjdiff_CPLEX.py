...
def solveMix(K,n,x0,ytrue,ycible,U,L,W,b,epsilon,epsilon_adv,relax,verbose=1):
    print("epsilon  : ", epsilon)

    m = Model(name='Glover_cplex')

    beta = {(j): m.binary_var(name='beta_{0}'.format(j)) for j in range(n[K]) if j!=ycible}
    ...
    for i in range(n[0]):
        m.add_constraint(epsilon >= z[0,i] - x0[i], "distance__{}".format(i))  
        m.add_constraint(epsilon >= x0[i] - z[0,i], "distance_{}".format(i)) 
    ...
    for k in range(1,K):
        for j in range(n[k]): 
            ...
            m.add_constraint(z[k,j] <= U[k-1][j] , "ReLU_neurone_{}_{}".format(k,j))

            m.add_constraint(z[k,j] >= m.sum(W[k-1][j][i] * z[k-1,i] for i in range(n[k-1])) + b[k-1][j], "ReLU_neurone_{}_{}".format(k,j))
            m.add_constraint((z[k,j] - m.sum(W[k-1][j][i] * z[k-1,i] for i in range(n[k-1])) + b[k-1][j]) * z[k,j] == 0, "ReLU_neurone_{}_{}".format(k,j))
    ...
    # ------------ FONCTION OBJECTIF ------------------------------------
    m.set_objective("min", z[K,ytrue] - z[K,ycible])
    m.print_information()
...
epsilon = 10
relax = 0
Sol, opt, status, execution_time = solveMix(K,n,x0,y0,ycible,U,L,W_reverse,b,epsilon,rho,relax,verbose=1)